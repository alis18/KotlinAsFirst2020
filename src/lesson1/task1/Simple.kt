...
fun angleInRadian(deg: Int, min: Int, sec: Int): Double {
    val minn = min.toDouble()
    val secc = sec.toDouble()
    val sum = deg + minn / 60 + secc / 3600
    return sum * PI / 180
}
...
fun travelMinutes(hoursDepart: Int, minutesDepart: Int, hoursArrive: Int, minutesArrive: Int): Int {
    val one = hoursDepart * 60 + minutesDepart
    val two = hoursArrive * 60 + minutesArrive
    return two - one
}
...
fun accountInThreeYears(initial: Int, percent: Int): Double {
    ...
    val x = a / 100 * percent + a
    val y = x / 100 * percent + x
    return y / 100 * percent + y
